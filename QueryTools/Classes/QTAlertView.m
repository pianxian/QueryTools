...
+ (QTAlertView *)sharedMask{
    static QTAlertView *alertView;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        if (!alertView) {
            alertView = [[QTAlertView alloc] init];
        }
    });
    return alertView;
}
...
- (void)addMaskLayer{
    ...
    [_maskLayer addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(maskTap)]];
...
-(void)setHideWhenTapOutSide:(BOOL)hideWhenTapOutSide{
    _hideWhenTapOutSide = hideWhenTapOutSide;
}
